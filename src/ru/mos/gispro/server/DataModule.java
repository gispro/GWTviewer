...
public class DataModule
{
	...
	public  static          String            TABLE_TREATMENT_CAT      = "public.treatments_cat";
	
	public  static  final   String            SQL_SELECT_REFERENCESS   = "select id, name from <0> where active = '1'";

	public  static  final   String            SQL_SELECT_TREATMENT_CAT = "select id, name from public.treatments_cat where id > 0";

	public  static  final   String            SQL_SELECT_TREATMENTS    = "select id, count, lon, lat from public.treatments"  ;
	public  static  final   String            SQL_SELECT_TREATMENT     = "select category, reported, email, content " +
			                                                                 "from public.treatments where id = "     ;
	...
	public  static  final   String            SQL_USER_INSERT          = "insert into <0> "                                  +
			                                                            ...
			                                                            "values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'guest', '1')";
	public  static  final   String            SQL_TREATMENT_INSERT     = "insert into treatments (id, lon, lat, category, "  +
			                                                                       "count, reported, email, content) "       +
                                                                            "values (?, ?, ?, ?, 1, ?, ?, ?)";
	public  static  final   String            SQL_TREATMENT_JOIN       = "update treatments set count = ? where id = ?";
	public  static  final   String   TREATMENT_DESC_TEMPL     = "{\"category\":\"<1>\",\"reported\":\"<2>\",\"email\":\"<3>\",\"content\":\"<4>\"}";
...
//			System.err.println("1. DataModule.Connect");
	...
	//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
	public static List<References> loadTreatmentCategories (Connection connect)
	{
		List<References> result = new ArrayList<References>();
		if (!Global.onIDE || (connect != null))
		{
			...
			{
				st = connect.createStatement();
				rs = st.executeQuery(SQL_SELECT_TREATMENT_CAT);
				while (rs.next())
				{
					References ref = new References (rs.getInt(1), rs.getString(2));
					result.add(ref);
			...
			Global.writeLog (Global.LOG_SECTION_DATAMODULE + "loadReferences : table = " + TABLE_TREATMENT_CAT + ", size = " + result.size());
		}
		return result;
	}
	//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
	public static List<Treatment> loadTreatments (Connection connect)
	{
		...
		if (!Global.onIDE || (connect != null))
		{
			...
			{
				st = connect.createStatement();
				rs = st.executeQuery(SQL_SELECT_TREATMENTS);
				while (rs.next())
				{
					result.add(new Treatment (rs.getInt(1), rs.getInt(2), rs.getDouble(3), rs.getDouble(4)));
				}
			} catch (SQLException e) {
				e.printStackTrace();
			}
	...
	//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
	public static String loadTreatmentDesc (Connection connect, int id)
	{
		String result = "";
		if (!Global.onIDE || (connect != null))
		{
			...
			try
			{
				String sql = SQL_SELECT_TREATMENT + String.valueOf(id);
				st = connect.createStatement();
				rs = st.executeQuery(sql);
				while (rs.next())
				{
					result = TREATMENT_DESC_TEMPL.replace("<1>", rs.getString(1));
					result = result.replace("<2>", rs.getString(2));
					result = result.replace("<3>", rs.getString(3));
					result = result.replace("<4>", rs.getString(4));
					break;
				}
			} catch (SQLException e) {
				e.printStackTrace();
	...
	//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
	public int insertTreatment (Connection connection, String lon, String lat, String category, String reported, String email, String content)
	{
		int result = -1;
		if (connection != null)
		{
			int id = getValue(connection, SQL_SELECT_MAX_ID, "public.treatments") + 1;
	    	PreparedStatement st = null;
			try
			{
				st = connection.prepareStatement(SQL_TREATMENT_INSERT);
				st.setInt    ( 1, id                 );
				st.setDouble ( 2, Double.valueOf(lon));
				st.setDouble ( 3, Double.valueOf(lat));
				st.setString ( 4, category           );
				st.setString ( 5, reported           );
	...
	//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
	public int joinTreatment (Connection connection, int id, int count, String email)
	{
		int result = -1;
		if (connection != null)
		{
			...
			try
			{
				st = connection.prepareStatement(SQL_TREATMENT_JOIN);
				st.setInt (1, count);
				st.setInt (2, id   );
				result = st.executeUpdate();
			} catch (SQLException e) {
