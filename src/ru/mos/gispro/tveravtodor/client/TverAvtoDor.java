...
public class TverAvtoDor implements EntryPoint
{
	...
	class NavigationHistory {
		...
		protected native JavaScriptObject test() /*-{
			...
			navigationHistory.onPreviousChange = function(state, lendth){
				this_.@ru.mos.gispro.tveravtodor.client.TverAvtoDor.NavigationHistory::update()();
		    };
...
//		    toolStrip2.addFormItem(treePickTreeItem);      //~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
        ...
        if (config.toolButtonFullExtent())
        {
		    ...
		    zoomFullExtent.setTooltip("Вся карта");
        }
...
/*
    ...
    {
        TreeNode[] nodes0 = treeGrid.getTree().getChildren(treeGrid.getTree().getRoot());
        String tmp = "0 - removeCheckBoxes : " +  nodes0.length + "\n";
        for (int i0 = 0; i0 < nodes0.length; i0++)
        {
            if (data.getChildren(nodes0[i0]).length > 0)
            {
                TreeNode[] nodes1 = treeGrid.getTree().getChildren(nodes0[i0]);
                tmp += "\t" + data.getChildren(nodes0[i0]).length + "\n";
                for (int i1 = 0; i1 < nodes1.length; i1++)
                {
                    if (data.getChildren(nodes1[i1]).length > 0)
                    {
                        tmp += "\t\t" + data.getChildren(nodes1[i1]).length + "\n";
//                        TreeNode[] nodes2 = treeGrid.getTree().getChildren(nodes1[i1]);
//                        for (int i2 = 0; i2 < nodes2.length; i1++)
//                        {
//                            if (data.getChildren(nodes2[i2]).length > 0)
//                            {
////                                tmp += "\t\t\t" + data.getChildren(nodes2[i2]).length + "\n";
//
//                                TreeNode[] nodes3 = treeGrid.getTree().getChildren(nodes2[i2]);
//                                for (int i3 = 0; i3 < nodes3.length; i1++)
//                                {
//                                    TreeNode node = nodes2[i2];
//                                    tmp += "\t\t\t\t" + node.getTitle();
//                                }
//                                break;
//                            }
//                        }
        ...
        com.google.gwt.user.client.Window.alert(tmp);
    }
*/
    //~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
}
