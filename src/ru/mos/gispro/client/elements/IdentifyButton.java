...
//        	            com.google.gwt.user.client.Window.alert("IdentifyButton.createHandlerRegistration : isSelected = " + isSelected());
//                      winModal.show();
...
//                            System.out.println ("createHandlerRegistration : mapService = " + mapService);
                            String name       = null;
                            if (mapService != null)
                            	name = mapService.getClass().getName();
//                          System.out.println ("createHandlerRegistration : name = " + name);
                            if (ArcGIS93.class.getName().equals(name))
                            {
                            	String objectInfo = getInfo();
                            	if ((((ArcGIS93) mapService).UrlIdentify() == null) ||
                            		(((ArcGIS93) mapService).UrlIdentify().length() == 0) || (objectInfo == null))
                            		continue;
                                ...
                                url += "geometryType=esriGeometryPoint&tolerance=7&" +
                                        ...
                                        ((ArcGIS93) mapService).Layers().replaceAll(", ", ",") + "&" + objectInfo;
//                                System.out.println ("IdentifyButton : url =\n" + url);
...
//                                    if (item.attributesByKey(keys.get(k)) != null)
                                    if (!item.attributesByKey(keys.get(k)).equalsIgnoreCase("null"))
                                        record.setAttribute("value", item.attributesByKey(keys.get(k)));
//            com.google.gwt.user.client.Window.alert("createWindow = " + keys.get(k) + ", " + item.attributesByKey(keys.get(k)));
                                }
                                data3.add(record);
                            }
                        }
//    					System.out.println ("IdentifyButton => GeometryManager : item.geometry() = " + item.geometry());
                        GeometryManager.clearGeometry();
