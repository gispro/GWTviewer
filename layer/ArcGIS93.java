...
public class ArcGIS93 implements MapService {
	...
	public ArcGIS93(String name, String url, String urlIdentify, String projection, TreeNode treeNode, Tree data, TreeGrid treeGrid) {
		this.name = name;
		this.url = url;
	...
	Map<String, List<LegendInfo>> images = new HashMap<String, List<LegendInfo>>();
	...
	public void invalidate() {
		if (layer == null) {
			...
			mapServiceInfo.legendsTrue(url,
					new AsyncCallback<Map<String, List<LegendInfo>>>() {
			...
			new JSONRequest().get(url + "?f=json",
									...
									if (legendInfos != null && legendInfos.size() != 0) {
										if (legendInfos.size() == 1)
											...
											for (LegendInfo li : legendInfos) {
												TreeNode legendNode = new TreeNode();
												legendNode.setAttribute("Layout", li.getLabel());
												legendNode.setID(idPrefix + Integer.toString(service.layers().get(i).id()) + "_" + Integer.toString(++index));
												legendNode.setIcon(li.getIcon());
												legendNode.setAttribute("isNodeGroup", false);
												legendNode.setEnabled(false);
												data.add(legendNode, test);
